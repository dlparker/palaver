...
class RescanListener:
    ...
    async def process_rescan(
        self,
        draft_event: DraftEndEvent,
        audio_buffer: AudioRingBuffer
    ) -> DraftEndEvent:
        """Encapsulated rescan processing for future queueing.

        Extracts audio segment, submits to WhisperWrapper, returns rescan result.
...
- Don't support multiple concurrent drafts (pipeline assumes serial event flow) - MVP stage
- Don't implement WebSocket reconnection logic - MVP stage
- Revision API must exist (Story 009) for POST to work
- Encapsulate rescan processing in method accepting (DraftEndEvent, AudioRingBuffer) for future queueing
...
- Create =process_rescan(draft_event: DraftEndEvent, audio_buffer: AudioRingBuffer)= method
  - Encapsulates rescan processing logic for future queueing
  - Extracts buffered audio segment [audio_start_time, audio_end_time]
  - Creates AudioStartEvent and AudioChunkEvents from buffered data
  - Submits to local WhisperWrapper
  - Returns rescan result (or raises exception)
- Call =process_rescan()= from DraftEndEvent handler
...
**Pipeline architectural constraint:**
- Audio events flow serially in time order from single source
- WhisperWrapper expects sequential AudioChunkEvents (stream_offset, speech_offset)
- VADFilter state machine assumes single audio stream
- DraftMaker assumes single draft context in progress
- Interleaved audio from multiple sources would break timing assumptions

**Cannot support truly concurrent drafts without pipeline redesign.**

**Prototype solution:**
- Single state machine (IDLE → COLLECTING → RESCANNING)
- Process one draft at a time (natural serialization)
- Future: Add queueing mechanism if needed (queue draft + audio buffer pairs)

**Encapsulation for future queueing:**
- Encapsulate rescan processing: =process_rescan(draft_event, audio_buffer)=
- Method accepts DraftEndEvent and AudioRingBuffer as arguments
- Returns rescan result (new DraftEndEvent)
- Enables future queue-and-process pattern without redesigning pipeline
- Queue can serialize multiple rescan requests while maintaining single-draft pipeline constraint

**Why encapsulation matters:**
- Clean separation: rescan logic isolated from event routing
- Testable: can test rescan logic independently
- Queueable: easy to wrap in queue consumer later
- Reusable: same logic works for file-based and network-based rescan
...
*** Rescan Queue (if multiple drafts needed)

If rapid draft creation exceeds rescan capacity:
- Create =RescanQueue= class wrapping =process_rescan()=
- Queue accepts =(draft_event, audio_buffer)= tuples
- Worker processes queue serially (maintains pipeline constraint)
- Max queue size prevents memory exhaustion
- Metrics: queue depth, processing time, drops

Implementation:
#+begin_src python
class RescanQueue:
    async def enqueue(self, draft_event: DraftEndEvent, audio_buffer: AudioRingBuffer):
        """Add draft to processing queue."""
        if len(self.queue) >= self.max_size:
            logger.warning(f"Queue full, dropping draft {draft_event.draft.draft_id}")
            return
        await self.queue.put((draft_event, audio_buffer.copy()))

    async def worker(self):
        """Process queue serially."""
        while True:
            draft_event, audio_buffer = await self.queue.get()
            try:
                result = await self.process_rescan(draft_event, audio_buffer)
                await self.send_revision(result)
...
**Key insight:** Encapsulation in =process_rescan()= makes queueing a simple wrapper, not a redesign.
